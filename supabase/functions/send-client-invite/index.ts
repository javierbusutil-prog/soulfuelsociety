...
serve(async (req) => {
  ...
  try {
    ...
    const { email, full_name, note } = await req.json();
    if (!email || !full_name) {
      return new Response(
    ...
    const noteBlock = note
      ? `
        <div style="margin: 24px 0; padding: 16px 20px; background: hsl(60, 20%, 97%); border-left: 3px solid hsl(213, 31%, 26%); border-radius: 4px;">
          <p style="font-size: 14px; line-height: 1.6; color: hsl(0, 0%, 25%); margin: 0; font-style: italic;">
            "${String(note).replace(/</g, "&lt;").replace(/>/g, "&gt;")}"
          </p>
          <p style="font-size: 12px; color: hsl(0, 0%, 50%); margin: 8px 0 0;">— Your coach</p>
        </div>
      `
      : "";
      ...
      <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 560px; margin: 0 auto; padding: 40px 24px; background: #ffffff;">
        ...
        <div style="margin-bottom: 16px;">
          <p style="font-size: 15px; line-height: 1.6; color: hsl(0, 0%, 25%);">
            Hey ${firstName}! You've been personally invited to join <strong>Soul Fuel Society</strong> — a space built for women who train with intention.
          </p>
        </div>
