...
import { Calculator, Save, RefreshCw, ChevronDown, ChevronUp, Droplets, Flame, Beef, Wheat, Salad } from 'lucide-react';
import { useMacroTargets, MacroTargets } from '@/hooks/useMacroTargets';
...
export function MacroCalculator() {
  ...
  const results = useMemo(() => {
    ...
    return { calories, proteinG, carbG: Math.max(carbG, 0), fatG, fiberG, waterOz, methodUsed };
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <div className="flex rounded-xl border border-input overflow-hidden">
                    <button
                      onClick={() => setUnit('lbs')}
                      className={`px-3 py-2 text-sm font-medium transition-colors ${unit === 'lbs' ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:text-foreground'}`}
                    >lbs</button>
                    <button
                      onClick={() => setUnit('kg')}
                      className={`px-3 py-2 text-sm font-medium transition-colors ${unit === 'kg' ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:text-foreground'}`}
                    >kg</button>
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Input
                        ...
                        placeholder={heightUnit === 'in' ? '65' : '165'}
                        className="flex-1"
                      />
                      <div className="flex rounded-xl border border-input overflow-hidden">
                        <button
                          onClick={() => setHeightUnit('in')}
                          className={`px-3 py-2 text-sm font-medium transition-colors ${heightUnit === 'in' ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:text-foreground'}`}
                        >in</button>
                        <button
                          onClick={() => setHeightUnit('cm')}
                          className={`px-3 py-2 text-sm font-medium transition-colors ${heightUnit === 'cm' ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:text-foreground'}`}
                        >cm</button>
                      </div>
                    </div>
                  </div>
