...
export function GrantProgramAccessDialog({ open, onOpenChange, memberId, memberName, coachId, onSuccess, }) {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const ownedIds = new Set((owned || []).map((o) => o.ebook_id));
            setPrograms((progs || []).filter((p) => !ownedIds.has(p.id)));
            setLoading(false);
        })();
    }, [open, memberId]);
    useEffect(() => {
    ...
    const handleSave = async () => {
        ...
        if (purchaseErr) {
            ...
            if (msg.includes('duplicate') || msg.includes('unique') || purchaseErr.code === '23505') {
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-1.5">
            ...
            : programs.length === 0
              ...
              <SelectContent>
                {programs.map((p) => (<SelectItem key={p.id} value={p.id}>
                    {p.title}
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input type="number" min="0" step="0.01" placeholder="0.00" value={amount} onChange={(e) => setAmount(e.target.value)} className="pl-7"/>
            </div>
          </div>
